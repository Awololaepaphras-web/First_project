...
const PointTransfer: React.FC<PointTransferProps> = ({ user }) => {
  ...
  const handleTransfer = async () => {
    if (!selectedUser || amount < 5000) {
      if (amount < 5000) {
        setMessage({ type: 'error', text: 'Minimum transfer amount is 5,000 points.' });
      }
      return;
    }
    if (amount > (user.points || 0)) {
      setMessage({ type: 'error', text: 'Insufficient points in your vault.' });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        setMessage({ type: 'success', text: `Successfully transferred ${amount} points to ${selectedUser.name}.` });
        setAmount(0);
            ...
            <div className="bg-gray-900 p-8 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-brand-proph mt-1">Available Points</p>
                </div>
            ...
            <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm space-y-6">
               ...
               <ul className="space-y-4">
                  {[
                    ...
                    'Minimum transfer amount is 5,000 points',
