...
import { 
  ...
  Search, Bell, User, Users, UserPlus, Send, MoreHorizontal, Home, Tv, Library, Star,
...
const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, notifications = [], onSelectTrend, appLogo, onSaveConfig, config }) => {
  ...
  const navItems = [
    ...
    { name: 'Parallel Universe', path: '/community', icon: <Zap className="w-6 h-6" /> },
    { name: 'Group Work', path: '/group-work', icon: <Users className="w-6 h-6" /> },
    { name: 'University Feed', path: '/university-feed', icon: <GraduationCap className="w-6 h-6" /> },
    ...
    {name: 'Referrals', path: '/referrals', icon: <UserPlus className="w-6 h-6" /> },
    {name: 'Leaderboard', path: '/leaderboard', icon: <Trophy className="w-6 h-6" /> },
    {name: 'Status', path: '/statuses', icon: <Camera className="w-6 h-6" /> },
    { name: 'Squads', path: '/chat', icon: <MessageSquare className="w-6 h-6" /> },
